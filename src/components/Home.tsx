...
export function Home() {
  ...
  // Filtrar imagens específicas solicitadas pelo usuário
  ...
  const specificImages = allProducts
    .filter(p => targetProducts.some(target => p.name.toLowerCase().includes(target)))
    .map(p => p.imageUrl)
    .filter((url): url is string => !!url);

  // Se encontrar as imagens específicas, usa elas. Senão, usa os destaques ou as imagens de Natal.
  const featuredImages = featured.filter(item => item.imageUrl).map(item => item.imageUrl).slice(0, 4);

  // Brigadeiro Fino sempre incluído
  const brigadeiroFinoUrl = 'https://ik.imagekit.io/q9ryn7hij/Card%C3%A1pio/Brigadeiros%20Especiais/Brigadeiro%20Fino.png?updatedAt=1763313912037';

  let heroImages: string[] = [];
  if (specificImages.length > 0) {
    heroImages = [...specificImages.filter((url): url is string => !!url), brigadeiroFinoUrl];
  } else if (featuredImages.length >= 4) {
    heroImages = [...featuredImages.filter((url): url is string => !!url), brigadeiroFinoUrl];
  } else {
    heroImages = [
      brigadeiroFinoUrl,
      'https://images.unsplash.com/photo-1545015451-f05567aa6bcc?auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1576618148400-f54bed99fcfd?auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1607269829858-32494941c451?auto=format&fit=crop&q=80',
      'https://images.unsplash.com/photo-1512504778165-1e4352656b7f?auto=format&fit=crop&q=80'
    ];
  }
        ...
        <div className="relative h-[500px] sm:h-[600px] overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-8 h-full flex items-center">
            <div className="relative z-1 max-w-2xl">
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-normal mb-6" style={{ fontFamily: 'Croissant One' }}>
                <span className="relative inline-block text-red-600">
                  D
                  <div className="absolute -top-4 -left-2 w-9 sm:-top-6 sm:-left-2 sm:w-14 transform -rotate-12 filter drop-shadow-md pointer-events-none">
                    <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                      {/* Parte vermelha */}
                      <path d="M80 68 Q 70 5 10 45 L 20 68" fill="#FF0000" stroke="#FF0000" strokeWidth="2" />
                      {/* Base branca do gorro (mais estreita) */}
                      <path d="M20 75 Q 50 82 80 75 L 80 65 Q 50 72 20 65 Z" fill="#FFFFFF" />
                      {/* Pom-pom */}
                      <circle cx="10" cy="45" r="10" fill="#FFFFFF" />
                    </svg>
                  </div>
                </span>
                <span className="text-red-600">oce </span>
              ...
              <p className="text-2xl text-gray-700 mb-8">
                ...
                <span className={`changing-word ${currentWord % 2 === 0 ? 'text-red-600' : 'text-green-600'}`}>{changingWords[currentWord]}</span>
              </p>
              <button
                onClick={() => {
